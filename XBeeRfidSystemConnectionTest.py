...
class XBeeSystemConnectionTest(jmri.jmrit.automat.AbstractAutomaton) :
    ...
    def __init__(self) :
        ...
        self.Xbee = self.tc.getNodeFromAddress(2) # change the address to that of a suitable node.
        self.xbeestream = self.Xbee.getIOStream()

        # set up an Rfid connection as a test.
        self.rfidstreamport = jmri.jmrix.rfid.RfidStreamPortController(self.xbeestream.getInputStream(), self.xbeestream.getOutputStream(), "test")
        self.rfidstreamport.configure()
        self.rfidstreamport.getSystemConnectionMemo().setProtocol(jmri.jmrix.rfid.protocol.coreid.CoreIdRfidProtocol());
...
a.setName("XBeeSystemConnectionTest RFID Script")
